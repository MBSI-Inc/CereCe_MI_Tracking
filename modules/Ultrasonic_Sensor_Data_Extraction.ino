...
float changeThreshold = 1.0; // cm - only notify if distance changes by more than this, tune as needed
...
{ // Logic to determine if Power on/off and light LED if needed
  ...
  {                                             // If difference between right now and last state change is greater than the green line on our diagram. If it's less than debounce is occuring (a.k.a flickering) so don't change.
    byte buttonState = digitalRead(BUTTON_PIN); // Current button state
    if (buttonState != lastButtonState)
    {                                        // If button state has changed (press/release occured)
  ...
  if (ledState == HIGH)
  { // Ultrasonic Measurements only occur if 'power' is on
