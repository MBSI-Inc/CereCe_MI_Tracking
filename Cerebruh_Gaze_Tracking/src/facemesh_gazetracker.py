...
try:
    ...
    print("Failed to import unity packages.")
...
class GazeTracker:
    ...
    def __init__(self, config_path, cam_index=0, smoothing_type="EMA", no_opencv=False):
        ...
        if not no_opencv:
            self.cam = self.set_up_camera(cam_index)
    ...
    def set_up_camera(self, cam_index):
        ...
        for backend, name in backends:
            try:
                candidate = cv2.VideoCapture(cam_index, backend) if backend is not None else cv2.VideoCapture(cam_index)
            except Exception:
                continue
            if not candidate.isOpened():
                ...
                continue
            # macOS cameras need several frames to warm up before read() succeeds
            ...
            for _ in range(5):
                ...
                time.sleep(0.1)
            if ret and frame is not None:
                print(f"[gaze_tracker] camera opened index={cam_index} backend={name}")
                cam = candidate
                break
            try: candidate.release()
            except Exception: pass
        ...
        if cam is None:
            if sys.platform == "darwin":
                platform_hint = (
                    "On macOS: grant camera access to Terminal / Python via "
                    "System Settings → Privacy & Security → Camera, "
                    "then close any other app using the camera (FaceTime, Zoom, etc.)."
                )
            else:
                platform_hint = "Check Windows Privacy Settings and close other apps using the camera."
            raise RuntimeError(
                f"GazeTracker could not open camera index {cam_index}. {platform_hint}"
            )
    ...
    def data_to_dict(self, combined_gaze_horizontal, combined_gaze_vertical,
                     blinked, face_detected=True, move_enabled=True,
                     yaw=0.0, pitch=0.0, roll=0.0):
        # _yaw / _pitch / _roll are consumed by the dashboard's head-rotation
        # chart and the Yaw/Pitch/Roll raw-metrics row. The real tracker
        # computes these internally (Util.get_head_rotation) but used to
        # drop them on the floor; passing them through here lights up the
        # head-pose graph that was drawing constant zeros before.
        output = {
    ...
    def get_gaze(self):
        ...
        # NOTE: the third value is 0.0 (float) not 0 (int) so pandas 2.x
        # infers the derivative column as float64 from the start. Without
        # this, `bTable.iat[-1, -1] = <float>` below raises TypeError on
        # pandas >= 2.0 ("Invalid value '...' for dtype 'int64'").
        row_data = [current_time, eyeslit_h_l / nosebridge_h * 100, 0.0]
        if bTable is None:
       ...
       # Update the face camera UI (for debugging)
        ...
        return self.data_to_dict(
            combined_gaze_horizontal, combined_gaze_vertical, self.toggle_turn,
            yaw=raw_data.get("head_rotation_yaw", 0.0),
            pitch=raw_data.get("head_rotation_pitch", 0.0),
            roll=raw_data.get("head_rotation_roll", 0.0),
        )
