"""
This script runs a calibration program, and saves the user-adjusted left and right boundary 
values as well as blink threshold values to the config file.
An existing config.json file needs to be present in the directory where you run this script.
"""
import json
import time
import cv2
...
def do_calibration(data):
    ...
    while not finished:
        ...
        if landmark_points:
            ...
            frame, right_direction_ratio, _ = Util.get_eye_direction_ratio(frame, landmarks, True)
            ema_right = (ema_alpha * right_direction_ratio) + ((1 - ema_alpha) * ema_right)
            right_direction_ratio = ema_right

            frame, left_direction_ratio, _ = Util.get_eye_direction_ratio(frame, landmarks, False)
            ...
            if start_calibration:
                ...
                if elapsed_time < GET_READY_TIME + DURATION_EACH_STEP * 3:
                    print_text_to_screen(frame, "Keep looking at the green dot")
                if elapsed_time > GET_READY_TIME + DURATION_EACH_STEP * 3 and elapsed_time < GET_READY_TIME * 2 + DURATION_EACH_STEP * 4:
                    print_text_to_screen(frame, "Close left eye, open right eye")
                elif elapsed_time > GET_READY_TIME * 2 + DURATION_EACH_STEP * 4:
                ...
                if (elapsed_time > GET_READY_TIME + DURATION_EACH_STEP and
                    ...
                    cv2.putText(frame, "Keep looking at the green dot", (350, 50),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2, cv2.LINE_AA,)
